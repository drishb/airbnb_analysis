...
- [x] 7.0 Figure generation
    ...
    - [x] 7.2 Build the centroid bubble map function — one marker per retained neighbourhood at its mean lat/long, sized by listing count, coloured by indicator (req 57). `figures.centroid_bubble_map(retained, values, *, title, legend_label, name, categorical=False, category_labels=None)`. Marker area (not radius) tracks listing count; extent is computed from the points so Avalon (Catalina, ~35 km offshore, bottom of frame) is always on-frame. Indicator legend below/left, listing-count size key bottom-right, both outside the data because the geographic aspect makes the axes tall and narrow. `categorical=True` → tab10 + labelled legend for the cluster map.
    - [x] 7.3 Render bubble maps for median price, host HHI, tourism intensity, 30-night-minimum share, and cluster assignment (req 57). Wired in `run_analysis`: `map_median_price.png`, `map_host_hhi.png`, `map_tourism_intensity.png` (= `tour_reviews_per_month_mean`), `map_min_nights_30.png` (= `reg_min30_share`), `map_clusters.png`.
    - [x] 7.4 Render the hexbin density map over raw lat/long, independent of neighbourhood labels (req 58). `figures.hexbin_density(df)` → `hexbin_listing_density.png`, gridsize 60, `bins="log"`, `mincnt=1`. Extent from all listing coords, so the 169 Catalina listings show as their own southern cluster.
    - [x] 7.5 Render the `minimum_nights` histogram capped at 90, annotated at the spike at exactly 30 (req 16). `figures.min_nights_histogram(df)` → `hist_minimum_nights.png`. Dashed marker + annotation at 30 (7,974 listings, 24.1%), the Home-Sharing Ordinance exemption threshold; 266 listings above 90 noted in the title, not shown.
    - [x] 7.6 Render the capped-vs-uncapped revenue scatter (req 22). `figures.revenue_capped_vs_uncapped(retained)` → `scatter_revenue_capped_vs_uncapped.png`. Per retained neighbourhood, capped-median x vs uncapped-median y, 45° reference line — every point sits above it, the divergence the 30+ night segment introduces.
    - [x] 7.7 Verify no Voronoi or synthesised boundary geometry appears anywhere in the codebase (req 59). Grep for `voronoi|tessellat|delaunay|convexhull|alpha_shape|choropleth|shapefile|geojson` and for `geopandas|shapely|scipy.spatial` imports — **zero matches** outside comments/docs stating the pipeline does not do this.
    - [x] 7.8 Confirm all 10 figures render at 150 dpi and are legible at print size (req 60). All 10 PNGs present in `outputs/figures/`, PIL reports dpi=(150, 150) on every one, sizes ≈ 900–1375 px wide (6–9 in at 150 dpi). Rendered and eyeballed each.

- [x] 8.0 Output assembly, reproducibility, and documentation

    - [x] 8.1 Wire every stage into `run_analysis.py` in dependency order (req 65). ingest → text topics → neighbourhood table + CSV → regression (`write_summary` then `compare_neighbourhood_fixed_effects`, append order) + residual plot → clustering (standardise → weight → k-diagnostics + plot → fit → PCA → centroids → labels → summary) → threshold sensitivity → exclusion report → 5 maps + hexbin + histogram + revenue scatter → limitations + success metrics.
    - [x] 8.2 Confirm all artefacts land in `outputs/` per the PRD §6 layout (req 66). All seven §6 files + the 10 named figures present; `success_metrics.md` added (task 8.8, not in the §6 list but required by 8.8). No stray files (the `_repro_check.csv` scratch file 8.8 uses is unlinked in the same call).
    - [x] 8.3 Write `src/report.py` to generate `limitations.md` containing every item in PRD §10 (req 67). `report.write_limitations(df)` — §10 field table with live-recomputed figures, plus the §7.3 estimator caveats, the two success-metric misses, and the out-of-scope note. Metric 11's check greps it for every §10 field name.
    - [x] 8.4 Pin all dependency versions in `requirements.txt` (req 68). 8 pins; all match the installed `.venv` exactly (pandas 3.0.2, numpy 2.4.4, scikit-learn 1.8.0, scipy 1.18.1, statsmodels 0.14.6, matplotlib 3.10.8, tabulate 0.10.0, pytest 8.4.2).
    - [x] 8.5 Seed NumPy, scikit-learn, and NMF initialisation globally, then verify two consecutive runs produce byte-identical CSV output (req 69, success metric 10). `set_seeds()` (random + numpy) plus `random_state=config.RANDOM_SEED` on every KMeans/PCA/NMF. Two full `run_analysis.py` runs from a deleted `outputs/`: **all text artefacts byte-identical**, not just the CSV. `regression_summary.txt` made deterministic by blanking the statsmodels `Date:` / `Time:` fields (`regression._deterministic_summary`). All `write_text` / `to_csv` calls forced to `encoding="utf-8"` (Windows was writing cp1252 for the em-dashes).
    - [x] 8.6 Write `README.md` with setup steps, including one-time `nltk.download()` if NLTK is used (req 63). Rewritten for the finished pipeline. NLTK is **not** used — scikit-learn's built-in `ENGLISH_STOP_WORDS`; README states no `nltk.download()` and no network is needed.
    - [x] 8.7 Verify the pipeline runs with no network access after environment setup (req 64). No `requests` / `urllib` / `nltk.download` / socket use anywhere in `src/` or `run_analysis.py`. Ran the full pipeline with `socket.socket.connect` / `create_connection` monkey-patched to raise → completed, all artefacts written.
    - [x] 8.8 Check every success metric in PRD §8 and record pass or fail for each. `report.write_success_metrics(...)` → `outputs/success_metrics.md`, each metric evaluated against the live pipeline objects / on-disk artefacts. **9 of 11 met.** FAIL: metric 3 (hedonic R² = 0.330 < 0.35) and metric 4 (cluster silhouette = 0.194 < 0.25) — both known, documented in `regression_summary.txt` / `cluster_summary.md`, accepted (fixed req-43 formula; intrinsic weak separation). All others PASS.
